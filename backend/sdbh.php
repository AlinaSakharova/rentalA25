 ...
 */
     ...
     * @param $select_col - first col of table
     * @param $select_col2 - second col of table
     * @param $select_col3 - third col of table
     * @param $from - low limit
     * @param $amount - up limit (if not set, calculates the number of rows in the table)
     * @param $order_by - field to sort anser by
     * @param $order - "DESC" or Null
     * @param $deadlock_up - throw deadlock_exception out or restart query internaly
     * @param $lock_mode - LISH - LOCK IN SHARE MODE, FU - FOR UPDATE or Null - nothing
    ...
    public function mselect_cols(
        ...
        while(True) try{
            if (!isset($amount)){
                $amount = $this->count_rows_in_table($tbl_name);

            }
            $query=$this->query_exc("SELECT `$id`,`$select_col`, `$select_col2`,`$select_col3` FROM `$tbl_name`
				ORDER BY `$order_by` $order LIMIT ".intval($from).", ".intval($amount[0][0]).
                " $lock_str"
            );
    ...
    /**
     * Count a set of rows into a table
     * @param $tbl_name - table name
     * @param deadlock_up - throw deadlock exception or restart query automatically
     * @return query result as array of rows, each an associative array
     */
    public function count_rows_in_table($tbl_name, $deadlock_up = False){
        while(True) try{
            $query = $this->query_exc("SELECT COUNT(*) as `0` FROM `$tbl_name`"
			);
            break;
